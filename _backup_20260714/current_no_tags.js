...
function getPublishSettings(p) {
  ...
  const saved = p.publishSettings || {};
  // margins는 deep merge (Object.assign은 shallow)
  return {
    ...defaults,
    ...saved,
    margins: { ...defaults.margins, ...(saved.margins || {}) },
    coverOptions: { ...defaults.coverOptions, ...(saved.coverOptions || {}) },
    frontMatter: { ...defaults.frontMatter, ...(saved.frontMatter || {}) }
  };
}
...
async function generateCoverPreview(p, set) {
  ...
  let paperW = 148;
  let paperH = 210;
  if (set.paperSize === 'B6') { paperW = 128; paperH = 182; }
  ...
  const canvas = document.createElement('canvas');
  // 화면 미리보기용이므로 크기를 적당히 줄임 (150 DPI 상당으로)
  const scaleRatio = 0.5;
  canvas.width = Math.round(canvasW_mm * MM_TO_PX * scaleRatio);
  ...
  if (opts.logo && logoImg) {
    ...
    const frontLogoW_mm = opts.logoFrontSize ?? 20;
...
function showPodStudio() {
  ...
  // 판권지
  $('#podShowCopyright').checked = set.frontMatter?.showCopyright || false;
  $('#podCopyrightOptions').style.display = set.frontMatter?.showCopyright ? 'flex' : 'none';
  $('#podAuthor').value = set.frontMatter?.author || '';
  $('#podPublishDate').value = set.frontMatter?.publishDate || '';
...
async function podUpdateCoverPreview() {
  ...
  const tmpSet = {
    ...
    coverOptions: {
      ...
      logoFrontSize: parseFloat($('#podLogoFrontSize').value) || 20,
...
function podSaveSettings() {
  ...
  p.publishSettings = {
    ...
    frontMatter: {
      showCopyright: $('#podShowCopyright').checked,
...
$$('.pod-preview-tab').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    btn.classList.add('active');
    const isInner = btn.dataset.preview === 'inner';
    $('#podPreviewInner').style.display = isInner ? 'flex' : 'none';
    $('#podPreviewCover').classList.toggle('active', !isInner);
    $('#podPageToggleWrap').style.display = isInner ? 'flex' : 'none';
    if (!isInner) podUpdateCoverPreview();
  });
});
...
// 여백 등 입력 실시간 반영
['podMarginTop','podMarginBottom','podMarginInner','podMarginOuter','podBleed','podPaperSize', 'podFontSize', 'podLineHeight'].forEach(id => {
  const el = $('#' + id);
...
// 판권지 토글
$('#podShowCopyright').addEventListener('change', (e) => {
  $('#podCopyrightOptions').style.display = e.target.checked ? 'flex' : 'none';
});
...
async function exportPODCover() {
  ...
  startY += Math.round(10 * MM_TO_PX); // 제목과 저자 사이 여백
  for (let i = 0; i < author.length; i++) {
    if (author[i] === ' ') { startY += authorFontSize * 0.5; continue; }
    ctx.fillText(author[i], spineX + spineW_px / 2, startY);
    startY += authorFontSize * 1.1;
  ...
  if (logoImg) {
    ...
    // 앞표지 중앙 하단 로고 (표준 크기 20mm, 도련 포함 하단에서 15mm 위)
    const frontLogoW = Math.round(20 * MM_TO_PX);
    const frontLogoH = (logoImg.height / logoImg.width) * frontLogoW;
    const fLx = frontX + (frontCoverW - frontLogoW) / 2; 
    const fLy = canvas.height - Math.round((BLEED_MM + 15) * MM_TO_PX) - frontLogoH; 
    ctx.drawImage(logoImg, fLx, fLy, frontLogoW, frontLogoH);
  }
...
`;
  ...
  <div class="chapter matter-page" style="break-before: page; break-after: page; display: flex; flex-direction: column; justify-content: flex-end; height: 80vh; font-size: 9pt; font-family: 'KoPub Batang', serif; text-align: left; padding: 0 10mm 15mm 10mm;">
    ...
    <div style="display: grid; grid-template-columns: 70px 1fr; gap: 6px; margin-bottom: 20px;">
      <div style="color: #444;">발행일</div><div>${escapeHtml(pubSet.frontMatter.publishDate || '2026년 07월 13일')}</div>
      <div style="color: #444;">지은이</div><div>${escapeHtml(pubSet.frontMatter.author || '박소연')}</div>
      <div style="color: #444;">발행처</div><div>퍼플</div>
    </div>
    <div style="display: grid; grid-template-columns: 70px 1fr; gap: 6px; margin-bottom: 30px;">
      <div style="color: #444;">출판등록</div><div>제 300-2012-167 호 (2012년 09월 07일)</div>
      <div style="color: #444;">주 소</div><div>서울시 종로구 종로 1 (종로1가) 교보생명빌딩 15층 ㈜교보문고</div>
      <div style="color: #444;">대표전화</div><div>1544-1900</div>
      <div style="color: #444;">홈페이지</div><div>www.kyobobook.co.kr</div>
    </div>
    <div style="font-size: 8pt; color: #333; line-height: 1.6;">
      <p style="margin-bottom: 6px;">ⓒ ${escapeHtml(pubSet.frontMatter.author || '박소연')} ${new Date().getFullYear()}</p>

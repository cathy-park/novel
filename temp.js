...
$('#addFrontMatterBtn').onclick = addFrontMatter;
...
function getPublishSettings(p) {
  const defaults = { 
    ...
    coverOptions: {
      ...
      fillMode: false,
      bgColor: '#2c2c2c',
      spineWidthMm: calculateSpineWidth(p)
    },
    frontMatter: {
      showCopyright: false,
      author: '',
  ...
  return Object.assign(defaults, p.publishSettings || {});
}

function calculateSpineWidth(p) {
  // 간단한 추산식: 총 글자수 / 500자 (1페이지) * 0.05mm (페이지당 두께)
  const eps = p.episodes || [];
  const totalChars = eps.reduce((sum, e) => sum + (e.body ? e.body.replace(/<[^>]*>?/gm, '').length : 0), 0);
  const estimatedPages = Math.max(10, Math.ceil(totalChars / 500));
  return Math.max(1, Math.round(estimatedPages * 0.05 * 10) / 10); // 소수점 1자리
}

const MM_TO_PX_300DPI = 300 / 25.4;
let currentCoverImageObj = null;

async function generateCoverPreview(imgObj, p, set) {
  if(!imgObj) return null;
  const paperSize = $('#pubPaperSize').value;
  const spineWidthMm = parseFloat($('#pubSpineWidth').value) || calculateSpineWidth(p);
  const fillMode = $('#pubCoverFillMode').checked;
  const bgColor = $('#pubCoverBgColor').value;
  const bleedMm = 3;

  let widthMm = 148; let heightMm = 210;
  if (paperSize === 'B6') { widthMm = 128; heightMm = 182; }

  const totalWidthMm = (widthMm * 2) + spineWidthMm + (bleedMm * 2);
  const totalHeightMm = heightMm + (bleedMm * 2);

  const canvas = document.createElement('canvas');
  canvas.width = Math.round(totalWidthMm * MM_TO_PX_300DPI);
  canvas.height = Math.round(totalHeightMm * MM_TO_PX_300DPI);
  const ctx = canvas.getContext('2d');
  ...
  if (fillMode) {
    // 전체 채우기 모드 (Center Crop)
    const imgRatio = imgObj.width / imgObj.height;
    const canvasRatio = canvas.width / canvas.height;
    let renderW, renderH, offsetX, offsetY;

    if (imgRatio > canvasRatio) {
      renderH = canvas.height; renderW = imgObj.width * (canvas.height / imgObj.height);
      offsetX = (canvas.width - renderW) / 2; offsetY = 0;
    } else {
      renderW = canvas.width; renderH = imgObj.height * (canvas.width / imgObj.width);
      offsetX = 0; offsetY = (canvas.height - renderH) / 2;
    }
    ctx.drawImage(imgObj, offsetX, offsetY, renderW, renderH);
  } else {
    // 앞표지만 채우기 모드 (우측 영역)
    const frontStartX = Math.round((widthMm + spineWidthMm + bleedMm) * MM_TO_PX_300DPI);
    const frontWidth = Math.round((widthMm + bleedMm) * MM_TO_PX_300DPI);
    const frontHeight = canvas.height;
    
    // 앞표지 영역 Center Crop
    const imgRatio = imgObj.width / imgObj.height;
    const frontRatio = frontWidth / frontHeight;
    let renderW, renderH, cropX, cropY;
    
    if (imgRatio > frontRatio) {
      renderH = frontHeight; renderW = imgObj.width * (frontHeight / imgObj.height);
      cropX = frontStartX + (frontWidth - renderW) / 2; cropY = 0;
    } else {
      renderW = frontWidth; renderH = imgObj.height * (frontWidth / imgObj.width);
      cropX = frontStartX; cropY = (frontHeight - renderH) / 2;
    }
    
    // 클리핑(마스크) 처리하여 영역 밖으로 넘어가지 않게 방지
    ctx.save();
    ctx.beginPath();
    ctx.rect(frontStartX, 0, frontWidth, frontHeight);
    ctx.clip();
    ctx.drawImage(imgObj, cropX, cropY, renderW, renderH);
    ctx.restore();
  }

  return canvas.toDataURL('image/jpeg', 0.85); // 0.85 for slightly smaller base64 size
}

$('#pubIncludeCover').onchange = (e) => { $('#pubCoverOptions').style.display = e.target.checked ? 'flex' : 'none'; };
...
function updateCoverPreviewRender() {
  if(!currentCoverImageObj) return;
  const p = currentProject(); if(!p) return;
  const set = getPublishSettings(p);
  generateCoverPreview(currentCoverImageObj, p, set).then(b64 => {
...
$('#pubFrontCoverInput').onchange = (e) => {
  ...
  reader.onload = (event) => {
    const img = new Image();
    img.onload = () => { currentFrontCoverObj = img; };
...
$('#pubBackCoverInput').onchange = (e) => {
  ...
  reader.onload = (event) => {
    const img = new Image();
    img.onload = () => { currentBackCoverObj = img; };
    img.src = event.target.result;
  };
  reader.readAsDataURL(file);
};
...
$('#openPublishSettingsBtn').onclick = () => {
  ...
  // Cover options
  $('#pubCoverBgColor').value = set.coverOptions?.bgColor || '#2c2c2c';
  $('#pubSpineWidth').value = set.coverOptions?.spineWidthMm || calculateSpineWidth(p);
  $('#pubSpineCalcText').textContent = `(자동 계산 권장: ${calculateSpineWidth(p)}mm)`;
  $('#pubPublisherLogo').value = set.coverOptions?.logo || '';
  
  if(set.coverOptions?.frontOriginal) {
    const img = new Image();
    img.onload = () => { currentFrontCoverObj = img; };
    img.src = set.coverOptions.frontOriginal;
  } else {
    currentFrontCoverObj = null;
    $('#pubFrontCoverInput').value = '';
  }

  if(set.coverOptions?.backOriginal) {
    const img = new Image();
    img.onload = () => { currentBackCoverObj = img; };
    img.src = set.coverOptions.backOriginal;
  } else {
    currentBackCoverObj = null;
    $('#pubBackCoverInput').value = '';
  }

  // Front Matter options
  $('#pubShowCopyright').checked = set.frontMatter?.showCopyright || false;
  $('#pubCopyrightOptions').style.display = set.frontMatter?.showCopyright ? 'flex' : 'none';
  $('#pubAuthor').value = set.frontMatter?.author || '';
  $('#pubPublisher').value = set.frontMatter?.publisher || '';
  $('#pubPublishDate').value = set.frontMatter?.publishDate || '';
  $('#pubPubReg').value = set.frontMatter?.pubReg || '';
  $('#pubAddress').value = set.frontMatter?.address || '';
  $('#pubPhone').value = set.frontMatter?.phone || '';
  $('#pubHomepage').value = set.frontMatter?.homepage || '';
...
$('#savePublishSettingsBtn').onclick = () => {
  ...
  p.publishSettings = {
    ...
    coverOptions: {
      ...
      bgColor: $('#pubCoverBgColor').value,
      logo: $('#pubPublisherLogo').value,
      spineWidthMm: parseFloat($('#pubSpineWidth').value) || calculateSpineWidth(p)
    },
    frontMatter: {
      showCopyright: $('#pubShowCopyright').checked,
      author: $('#pubAuthor').value,
      publisher: $('#pubPublisher').value,
      publishDate: $('#pubPublishDate').value,
      pubReg: $('#pubPubReg').value,
      address: $('#pubAddress').value,
      phone: $('#pubPhone').value,
      homepage: $('#pubHomepage').value
    }
  };
  ...
  try {
    ...
    console.warn('Failed to save publish settings to localStorage', e);
  }
...
$('#exportPODCoverBtn').onclick = exportPODCover;
...
async function exportPODCover() {
  ...
  // 책등 텍스트 그리기 (가로 정렬, 세로로 회전)
  const title = p.title || '제목 없음';
  const author = set.frontMatter?.author || '저자';
  
  ctx.save();
  ctx.translate(spineX + spineW_px / 2, Math.round((BLEED_MM + 20) * MM_TO_PX)); // 상단 여백 20mm
  ctx.rotate(Math.PI / 2); // 90도 회전
  
  ctx.fillStyle = (opts.bgColor || '#2c2c2c').toLowerCase() === '#ffffff' ? '#000' : '#fff'; // 배경이 흰색이면 검은 글씨
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  
  // 제목 폰트
  ctx.font = `bold ${Math.round(4 * MM_TO_PX)}px 'KoPub Batang', serif`;
  ctx.fillText(title, 0, 0);
  
  // 저자 폰트
  const titleWidth = ctx.measureText(title).width;
  ctx.font = `normal ${Math.round(3 * MM_TO_PX)}px 'KoPub Batang', serif`;
  ctx.fillText(author, titleWidth + Math.round(10 * MM_TO_PX), 0);
  
  ctx.restore();

  // 로고 그리기 (책등 하단, 앞표지 우측 하단)
  if (logoImg) {
    // 책등 하단 로고 (너비를 책등에 맞추되 패딩 2mm)
    const spineLogoW = spineW_px - Math.round(4 * MM_TO_PX); 
    const spineLogoH = (logoImg.height / logoImg.width) * spineLogoW;
    const sLx = spineX + Math.round(2 * MM_TO_PX);
    const sLy = canvas.height - Math.round((BLEED_MM + 15) * MM_TO_PX) - spineLogoH;
    ctx.drawImage(logoImg, sLx, sLy, spineLogoW, spineLogoH);
    
    // 뒷표지 하단 로고
    const backLogoW = Math.round(25 * MM_TO_PX);
    const backLogoH = (logoImg.height / logoImg.width) * backLogoW;
    const bLx = backCoverW - Math.round(15 * MM_TO_PX) - backLogoW; // 우측 여백 15mm
    const bLy = canvas.height - Math.round((BLEED_MM + 15) * MM_TO_PX) - backLogoH; // 하단 여백 15mm
    ctx.drawImage(logoImg, bLx, bLy, backLogoW, backLogoH);
  }
...
`;
  ...
  // 1. 표지 (옵션) - 표지는 분리되어 이미지로 다운로드되므로 PDF에는 포함하지 않습니다.

  // 2. 판권지 (옵션)
  if (pubSet.frontMatter?.showCopyright) {
    html += `
  <div class="chapter matter-page" style="break-before: page; break-after: page; display: flex; flex-direction: column; justify-content: flex-end; height: 80vh; font-size: 9pt; font-family: 'KoPub Batang', serif; text-align: left; padding: 0 10mm 15mm 10mm;">
    <h2 style="font-size: 16pt; margin-bottom: 30px; font-weight: 700;">${escapeHtml(p.title)}</h2>
    <div style="display: grid; grid-template-columns: 70px 1fr; gap: 6px; margin-bottom: 20px;">
      <div style="color: #444;">발행일</div><div>${escapeHtml(pubSet.frontMatter.publishDate || '2026년 00월 00일')}</div>
      <div style="color: #444;">지은이</div><div>${escapeHtml(pubSet.frontMatter.author || '저자명 미상')}</div>
      <div style="color: #444;">발행처</div><div>${escapeHtml(pubSet.frontMatter.publisher || '독립출판')}</div>
    </div>
    <div style="display: grid; grid-template-columns: 70px 1fr; gap: 6px; margin-bottom: 30px;">
      <div style="color: #444;">출판등록</div><div>${escapeHtml(pubSet.frontMatter.pubReg || '제 000-0000-000 호 (0000년 00월 00일)')}</div>
      <div style="color: #444;">주 소</div><div>${escapeHtml(pubSet.frontMatter.address || '주소를 입력하세요')}</div>
      <div style="color: #444;">대표전화</div><div>${escapeHtml(pubSet.frontMatter.phone || '0000-0000')}</div>
      <div style="color: #444;">홈페이지</div><div>${escapeHtml(pubSet.frontMatter.homepage || 'www.example.com')}</div>
    </div>
    <div style="font-size: 8pt; color: #333; line-height: 1.6;">
      <p style="margin-bottom: 6px;">ⓒ ${escapeHtml(pubSet.frontMatter.author || '저자명 미상')} ${new Date().getFullYear()}</p>
      <p>본 책 내용의 전부 또는 일부를 재사용하려면<br>반드시 저작권자의 동의를 받으셔야 합니다.</p>
    </div>
  </div>`;
  }
  ...
  beforeTocEps.forEach(ep => {
    ...
    html += `
  <div class="chapter matter-page" id="ep-${ep.id}">
    <div class="chapter-content ql-editor">${processed.body}</div>
  </div>`;
  });

  // 4. 제목 페이지 (자동 생성)
  // 사용자가 직접 앞부속을 만들지 않았을 때만 제목 페이지를 생성
  if (beforeTocEps.length === 0) {
    html += `
  <div class="title-page">
    <h1>${escapeHtml(p.title)}</h1>
    <p style="margin-top: 20px; font-size: 14pt;">${escapeHtml(pubSet.frontMatter?.author || '')}</p>
  </div>
`;
  }
  ...
  if (pubSet.autoTOC && tocEps.length > 0) {
    html += `
  <div class="toc-page">
    <h2>목차</h2>
    <ul class="toc-list">
`;
    tocEps.forEach((ep, i) => {
      const displayTitleHtml = getEpisodeDisplayTitle(ep, p, true);
      html += `<li>
        <a href="#ep-${ep.id}" class="toc-title">${displayTitleHtml}</a>
      </li>`;
    });
    html += `
    </ul>
